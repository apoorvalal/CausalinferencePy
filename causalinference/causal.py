...
class CausalModel(object):
    ...
    def est_propensity_s(self, lin_B=None, C_lin=1, C_qua=2.71):
        ...
        self.propensity = PropensitySelect(self.raw_data, lin_basic, C_lin, C_qua)
        self.raw_data._dict["pscore"] = self.propensity["fitted"]
        self._post_pscore_init()
    ...
    def est_via_weighting(
        self, estimand="ate", use_covariates=False, final_est="dim", hajekize=False
    ):
        ...
        self.estimates["weighting"] = Weighting(
            self.raw_data,
            estimand,
            use_covariates,
            final_est=final_est,
    ...
    def est_via_matching(self, weights="inv", matches=1, bias_adj=False):
        ...
        self.estimates["matching"] = Matching(self.raw_data, W, matches, bias_adj)
...
def select_cutoff(g):
    ...
    if g.max() <= 2 * g.mean():
        ...
        cutoff = 0.5 - np.sqrt(0.25 - 1.0 / gamma)
...
def select_blocks(pscore, logodds, D, K, p_low, p_high):
    ...
    return select_blocks(pscore, logodds, D, K, low, mid) + select_blocks(
        pscore, logodds, D, K, mid, high
    )
