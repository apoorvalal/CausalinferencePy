...
class Propensity(Dict):
    ...
    def __str__(self):
        ...
        entries1 = ["", "Coef.", "S.e.", "z", "P>|z|", "[95% Conf. int.]"]
        ...
        for (lin_term, coef, se) in zip(lin, coefs[1:], ses[1:]):
            entries3 = tools.gen_reg_entries("X" + str(lin_term), coef, se)
            output += tools.add_row(entries3, entry_types2, col_spans2, table_width)

        qua = self._dict["qua"]
        lin_num = len(lin) + 1  # including intercept
        for (qua_term, coef, se) in zip(qua, coefs[lin_num:], ses[lin_num:]):
...
def calc_coef(X_c, X_t):
    ...
    logit = fmin_bfgs(neg_ll, np.zeros(K), neg_grad, full_output=True, disp=False)
